...
from owe.utils import load_checkpoint
...
class BertOWEEmbedderModule(nn.Module):
    """ Wrapper for BERT-OWE module to fit into jiant APIs. """

    def __init__(self, args, owe_vectors, cache_dir=None):
        super(BertEmbedderModule, self).__init__()
        ...
        self.mapper = Mapper(owe_vectors, len(owe_vectors[0]))
        self.mapper = torch.nn.DataParallel(self.mapper, device_ids=Config.get("GPUs"))
        ...
        if args.load_best:
            ...
            if checkpoint_owe:
                ...
                self.mapper.load_state_dict(checkpoint_owe["mapper"])
                log.info("Initialized OWE model, mapper and optimizer from the loaded checkpoint.")
    ...
    def forward(
        ...
        Returns:
            h: [batch_size, seq_len, d_emb]
        """
        owe_ids = sent["owe_pretokenized"]
        owe_input = torch.tensor(owe_ids, dtype=torch.long)
        enc = self.mapper(torch.zeros(len(owe_input), 1, dtype=torch.long), owe_input)
        assert "bert_wpm_pretokenized" in sent
        # <int32> [batch_size, var_seq_len]
        ids = sent["bert_wpm_pretokenized"]
        # BERT supports up to 512 tokens; see section 3.2 of https://arxiv.org/pdf/1810.04805.pdf
    ...
    def get_output_dim(self):
        if self.embeddings_mode == "cat":
            return 2 * self.model.config.hidden_size + 300
        else:
            return self.model.config.hidden_size + 300